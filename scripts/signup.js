$(document).ready(function () {
  ...
  var errors = [];
  errors["name"] =
    "&nbsp;User Name can contain only numeral, character, or _(underscore)";
  ...
  $('input[name$="wantusername"]').keyup(function () {
    validate_username();
  });
  $('input[name$="wantpassword"]').keyup(function () {
    validate_pass();
  });
  $('input[name$="passagain"]').blur(function () {
    validate_pass();
  });
  ...
  function validate_pass() {
    ...
    if (len < 6 || len > 16) {
      $(".pass1check")
        .html(cross + errors["passshort"])
        .show();
      return (cansubmit = false);
    } else if (
      $('input[name$="wantpassword"]').val() !=
      $('input[name$="passagain"]').val()
    ) {
      $(".pass2check")
        .html(cross + errors["pass2"])
        .show();
