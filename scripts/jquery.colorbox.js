...
(function ($, document, window) {
  ...
  publicMethod.init = function () {
    ...
    $("." + boxElement).live("click", function (e) {
      ...
      if (
        !(
          (e.button !== 0 && typeof e.button !== "undefined") ||
          e.ctrlKey ||
          e.shiftKey ||
          e.altKey
        )
  ...
  publicMethod.load = function (launched) {
    ...
    if (settings.inline) {
      ...
      $loadingBay.load(href, function (data, status, xhr) {
        prep(
          status === "error"
            ? $div("Error").text("Request unsuccessful: " + xhr.statusText)
            : $(this).contents()
        );
      });
    }
  ...
  publicMethod.next = function () {
    if (
      !active &&
      $related[1] &&
      (index < $related.length - 1 || settings.loop)
    ) {
